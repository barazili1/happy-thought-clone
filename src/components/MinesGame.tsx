...
export const MinesGame: React.FC<MinesGameProps> = ({ onBack, userId, platform }) => {
  ...
  return (
    <div className="flex flex-col min-h-full bg-transparent font-sans text-zinc-900" dir="rtl">
      <div className="relative z-10 flex flex-col px-4 pt-4 pb-12 max-w-md mx-auto w-full">
        {/* Top Bar */}
        <div className="flex items-center justify-between mb-5 p-3 bg-white/95 backdrop-blur-xl border rounded-2xl shadow-lg"
          style={{ borderColor: 'rgba(var(--primary-color-rgb),0.35)' }}
        >
          <button
            onClick={onBack}
            className="w-9 h-9 rounded-xl bg-zinc-50 border border-zinc-300 flex items-center justify-center transition-all active:scale-95"
            title="رجوع"
          >
        ...
        {/* Grid 5x5 */}
        <div className="p-3 rounded-3xl bg-white border border-zinc-300 shadow-[0_18px_40px_rgba(15,23,42,0.08)] mb-6 mx-auto">
        ...
        {/* Diamond count */}
        <div className="mb-5">
          <p className="text-center text-xs font-black text-zinc-600 mb-2.5">عدد الألماس</p>
            ...
            {DIAMOND_OPTIONS.map((n) => {
              ...
              return (
                <button
                  ...
                  className="rounded-xl border-2 bg-white/95 backdrop-blur-md font-black text-base transition-all active:scale-95 shrink-0"
                >
                  {n}
                </button>
              );
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <button
            onClick={handleStart}
            disabled={isRunning}
            className="h-12 rounded-xl bg-teal-600 text-white font-black flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50"
          >
